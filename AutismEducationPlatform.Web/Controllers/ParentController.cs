...
namespace AutismEducationPlatform.Web.Controllers
{
    ...
    public class ParentController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            // Gerçek verilerle güncellenecek
            var viewModel = new ParentDashboardViewModel
            {
                ChildInfo = await GetChildInfo(user.Id),
                Courses = await GetActiveCourses(user.Id),
                ProgressReport = await GetProgressReport(user.Id),
                ...
                UserProfile = new UserProfile
                {
                    ...
                    Phone = user.PhoneNumber ?? "Belirtilmemiş",
